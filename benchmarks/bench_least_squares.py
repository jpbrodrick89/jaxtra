...
for n_cols in COL_COUNTS:
  print(f"\nn_cols = {n_cols}")
  print(
    f"  {'n_rows':>7}  {'jaxtra (ms)':>12}  {'dense QR (ms)':>14}  "
  ...
  for n_rows in ROW_SIZES:
    ...
    t_jaxtra = time_jax_fn(
      _jaxtra_solve, A_jx, b_jx, n_warmup=N_WARMUP, n_repeat=N_REPEAT
    )
    t_dense = time_jax_fn(
      _dense_qr_solve, A_jx, b_jx, n_warmup=N_WARMUP, n_repeat=N_REPEAT
    )
    t_scipy = time_numpy_fn(
      _scipy_svd_solve, A_np, b_np, n_warmup=N_WARMUP, n_repeat=N_REPEAT
    )
...
with open(csv_path, "w", newline="") as fh:
  writer = csv.DictWriter(
    fh, fieldnames=["n_rows", "n_cols", "method", "time_ms"]
  )
  writer.writeheader()
...
for n_cols in COL_COUNTS:
  ...
  for label, color, marker in METHODS:
    ...
    ax.plot(
      xs, ys, label=label, color=color, marker=marker, linewidth=2, markersize=5
    )
